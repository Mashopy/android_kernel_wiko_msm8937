                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    FTS_INFO("The display-coords name: %s.", name);	
    if (!strcmp(name, "focaltech,display-coords") || !strcmp(name, "focaltech,display-coords-old"))
    {
       ...
       FTS_INFO("x_max:%d, y_max:%d.", coords[2], coords[3]);	
    }
    else
    {
...
if (bEnTGesture) {
    ...
    {
	  FTS_ERROR("Write 0xA5 register OK, ret=%d!", retval);
   }
    data->suspended = true;
    msleep(5);

//Add by Luoxingxing<xingxing.luo@tinno.com> for TP power contral.
#if FTS_SUSPEND_POWER_OFF
